...
//        System.out.println(modelId + Arrays.toString(mainMesh.translucentTexture));
    ...
    private static GeoModel buildMesh(RawYsmModel.RawGeometry rawGeo, GeometryDescription context, int textureCount, TranslucencyScanner scanner, boolean allCutout) {
        ...
        for (GeoModel.BakedBone b : bakedBones) {
            ...
            if (isLeftArmRenderBone(b.name)) b.partMask = 1;
            else if (isRightArmRenderBone(b.name)) b.partMask = 2;
...
//                }
    ...
    public static GeoModel buildMesh(GeoBone[] bones, Map<String, String> parentMap, GeometryDescription context, boolean[] translucencyArray) {
        ...
        boolean hasLeftArm = parentMap.keySet().stream().anyMatch(YSMClientMapper::isLeftArmRenderBone);
        boolean hasRightArm = parentMap.keySet().stream().anyMatch(YSMClientMapper::isRightArmRenderBone);
        boolean[] flags = new boolean[]{hasLeftArm, hasRightArm, parentMap.containsKey("Background")};
        return new GeoModel(bones, boneNameArrays, flags, context, translucencyArray);
    }

    private static boolean isLeftArmRenderBone(String name) {
        return name != null && (name.startsWith("LeftArm") || name.startsWith("LeftForeArm") || name.startsWith("LeftHand"));
    }

    private static boolean isRightArmRenderBone(String name) {
        return name != null && (name.startsWith("RightArm") || name.startsWith("RightForeArm") || name.startsWith("RightHand"));
    }
    ...
    public static GeometryDescription buildContext(RawYsmModel.RawGeometry model) {
        if (model == null) {
            return new GeometryDescription("geometry.unknown", 128, 128, 7, 9, new double[]{0, 1.5, 0});
        }
